...
const STORAGE_KEYS = {
  ...
  SESSION_META: "dls_study_session", // For progress tracking
...
export async function setSessionState(step: number, caseIndex: number): Promise<void> {
  ...
  if (error) {
    console.error("Failed to sync session state to Supabase:", error);
  }
}
...
export function isPatientComplete(caseId: string, answers: Record<string, AnswerValue>): boolean {
  const caseCfg = questionnaireConfig.segmentation_patients
...
export function isProfileComplete(answers: Record<string, AnswerValue>): boolean {
  return questionnaireConfig.profile_questions
    .filter(q => q.required)
    .every(q => {
...
export function getMissingQuestionsReport(answers: Record<string, AnswerValue>): CompletenessReport {
  ...
  questionnaireConfig.profile_questions.forEach(q => {
    if (q.required) {
  ...
  // Check Cases
  questionnaireConfig.segmentation_patients.forEach(p => {
...
async function upsertAnswerToSupabase(payload: UpsertAnswerPayload): Promise<void> {
  ...
  const { data, error, status, statusText } = await supabase.from("answers").upsert(
    dbPayload,
...
export async function saveSurveyProgress(
  ...
  for (const [key, answerObj] of Object.entries(answers)) {
    ...
    const step = parts[0];
    const isProfile = step === 'profile' || (parts.length > 1 && parts[1] === '__profile__');
    let caseId: string | null = null;
    let itemId: string;

    if (isProfile) {
      if (parts.length === 3 && parts[1] === '__profile__') {
        caseId = '__profile__';
        itemId = parts[2];
      } else {
        caseId = '__profile__';
  ...
  if (isSupabaseConfigured() && answersArray.length > 0) {
    try {
      ...
      // Queue offline if needed, simplified for clarity here
      return false;
    }
  } else {
...
export async function fetchAnswersFromSupabase(participantId?: string, sessionId?: string): Promise<Record<string, AnswerValue>> {
  ...
  const validCaseIds = new Set<string | null>([
    '__profile__', null,
    ...questionnaireConfig.segmentation_patients.flatMap(p => p.variants.map(v => v.case_id))
  ]);
...
export async function testManualSupabaseSave() {
  ...
  try {
    ...
    if (error) {
      ...
      const selectResult = await supabase.from('answers').select('*').eq('session_id', sessionId);
